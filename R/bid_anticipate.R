...
bid_anticipate <- function(
  ...
  if (!is.null(bias_mitigations)) {
    if (inherits(bias_mitigations, "bidux_bias_mitigations")) {
      ...
      cli::cli_abort(standard_error_msg(
        "bias_mitigations must be a bidux_bias_mitigations object or list",
