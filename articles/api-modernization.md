...
the definitive guide to modern API usage in bidux 0.4.0+**.
...
As of 0.4.0, the nested data_story format has been removed. Update your
code:
...
of bidux 0.4.0, the flat API is the ONLY supported approach** for
...
The nested format using `variables` and `relationships` parameters was
**REMOVED** in bidux 0.4.0. Code using this format will now error.

### Migration from Nested to Flat Format

If you’re upgrading from 0.3.x, update your code from the old nested
format to the flat format:
...
validation and modern S3 methods. **Always use the flat API for
data_story objects (required in 0.4.0+).**
...
compatibility, using
[`new_data_story()`](https://jrwinget.github.io/bidux/reference/new_data_story.md)
with the flat API is required for proper validation and consistency.
...
API patterns using the **flat API throughout** (required in 0.4.0+).
...
As of 0.4.0, the nested format is no longer supported and will produce
an error:
...
As of version 0.4.0, the nested data_story format has been removed. This
is a breaking change that requires updating existing code.

### What Was Removed

The following parameters and features are no longer supported:

1.  **Nested data_story format**: The `variables` and `relationships`
    parameters in
    [`new_data_story()`](https://jrwinget.github.io/bidux/reference/new_data_story.md)
    have been removed
2.  **Layout auto-selection based on nested format**: Removed (flat API
    only)
3.  **Layout-specific bias mitigations**: Removed in favor of explicit
...
**Step 1: Identify all
[`new_data_story()`](https://jrwinget.github.io/bidux/reference/new_data_story.md)
calls with variables/relationships**
...
All other enhancements from 0.3.x maintain backward compatibility. Only
the nested data_story format was removed.
...
- **Flat data_story API (REQUIRED)**: Intuitive
  `new_data_story(hook, context, tension, resolution)` is the only
  supported format
...
- **Nested data_story format** (variables/relationships): **REMOVED in
  0.4.0**
...
1.  Search your codebase for `variables = list(` and
    `relationships = list(`
2.  Convert all nested format calls to flat format (see migration
    examples above)
...
suitable for modern R workflows with a cleaner, more maintainable API.
