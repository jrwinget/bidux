...
generate_domain_insights <- function(bid_result, custom_concepts = NULL) {
  ...
  # Analyze suggestions against domain best practices
  suggestions <- bid_result$suggestions

  if (length(suggestions) > 0) {
    insights$suggestion_analysis <- paste(
      "Generated", length(suggestions),
      "concept groups with actionable recommendations."
    )
  }
