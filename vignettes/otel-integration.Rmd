...
The beauty of bidux's OTEL support is that the analysis workflow is **identical** to `shiny.telemetry`:
...
### Understanding OTEL-Sourced Issues

When bidux analyzes OTEL data, spans are automatically converted to the event schema. The `telemetry_source` column indicates the data origin:
...
# Check source
unique(issues$telemetry_source)
#> [1] "opentelemetry"

# OTEL data provides additional performance context
issues |>
  filter(issue_type == "delayed_interaction") |>
  select(problem, evidence, duration_ms)
#> Problem: Users take a long time before making their first interaction
#> Evidence: Median time to first input is 47 seconds
#> Duration: Performance data from OTEL spans
```
