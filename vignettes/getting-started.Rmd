...
bid_concept("hick's law") %>% select(concept, description)

# Partial matching
bid_concept("proximity") %>% select(concept, description)
```
...
notice_result %>% select(theory, suggestions)
```
...
interpret_result %>% select(central_question, hook, tension, resolution, suggestions)
```

The function evaluates our data story elements and provides suggestions for improvement.
...
structure_result <- bid_structure(
  ...
  layout = "dual_process",
  concepts = c("Principle of Proximity", "Default Effect", "Aesthetic-Usability")
)

structure_result %>% select(layout, concepts, suggestions)
```

The function provides layout-specific suggestions based on the psychological concepts we've chosen to apply.
...
anticipate_result %>% select(bias_mitigations, suggestions)
```

The function evaluates our bias mitigation strategies and provides implementation suggestions.
...
validate_result %>% select(summary_panel, collaboration, suggestions)
```
...
1. **Planning Phase**: Use the BID framework to document your design decisions before writing code
2. **Development Phase**: Reference your BID documentation to implement appropriate UI patterns
3. **Testing Phase**: Validate that your implementation addresses the issues identified in Stage 1
4. **Iteration Phase**: Update your BID documentation as you refine the dashboard
...
Visit [github.com/jrwinget/bidux](https://github.com/jrwinget/bidux) for updates and to contribute to the package.
