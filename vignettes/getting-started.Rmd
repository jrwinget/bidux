...
DiagrammeR::grViz("
  digraph bid_framework {
    ...
    # Allow edges to connect to subgraphs themselves
    graph [
      compound = true,
      fontname = 'Arial', 
      fontsize = 12, 
    ...
    # Dashed subgraph in the middle
    subgraph cluster0 {
      label = '';                 /* no label on the cluster */
      style = 'rounded,dashed';   /* dashed outline */
      color = 'black';            /* ensure border is visible */

      interpret  [label = 'INTERPRET'];
      structure  [label = 'STRUCTURE'];
      anticipate [label = 'ANTICIPATE'];

      /* Force INTERPRET up top, STRUCTURE & ANTICIPATE side-by-side below */
      { rank = min; interpret }
      { rank = same; structure; anticipate }

      /* Connect them in a triangle */
      interpret -> structure;
      structure -> anticipate;
    ...
    # Connect NOTICE into the cluster boundary
    notice -> interpret [lhead = cluster0, constraint = false];

    # Connect from the cluster boundary out to VALIDATE
    anticipate -> validate [ltail = cluster0, constraint = false];
  }
")
```
