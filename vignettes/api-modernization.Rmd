...
The bidux package has been modernized with enhanced API consistency, better error handling, and optional tibble support. **This vignette is the definitive guide to modern API usage in bidux 0.3.2+**.
...
The `new_data_story()` function provides a structured way to create behavioral insights. **As of bidux 0.3.2, the flat API is the RECOMMENDED approach** for creating data stories.
...
The nested format using `variables` and `relationships` parameters is **DEPRECATED** and will be removed in bidux 0.4.0.

```{r data_story_deprecated, eval = FALSE}
...
The enhanced `bid_interpret()` function captures user needs and central questions with comprehensive validation and modern S3 methods. **Always use the recommended flat API for data_story objects.**
...
**Note:** While `bid_interpret()` still accepts plain list format for `data_story` for backward compatibility, using `new_data_story()` with the flat API is recommended for clarity and consistency.
...
Demonstrate the full modernized BID framework workflow with consistent API patterns using the **RECOMMENDED flat API throughout**.
...
```{r deprecation_warning, eval = FALSE}
...
All enhancements maintain full backward compatibility - existing code continues to work with deprecation warnings where appropriate.

```{r backward_compatibility, eval = FALSE}
...
1. **Immediate (0.3.2)**: Code continues to work with deprecation warnings
2. **Short-term**: Update to flat API following warning guidance
3. **Before 0.4.0**: Complete migration required (nested format will be removed)
...
- **Flat data_story API (RECOMMENDED)**: Intuitive `new_data_story(hook, context, tension, resolution)` replaces nested format
...
- **Nested data_story format** (variables/relationships): Deprecated in 0.3.2, removal in 0.4.0
...
These improvements make the package more robust, user-friendly, and suitable for modern R workflows while maintaining all existing functionality during the transition period.
